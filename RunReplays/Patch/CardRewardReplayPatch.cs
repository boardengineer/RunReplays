...
public static class CardRewardReplayPatch
{
    ...
    private static Node? FindHolderByTitle(
        Array<Node> nodes, string expectedTitle)
    {
    ...
    internal static bool SelectCard(string expectedTitle)
    {
        ...
        var match = FindHolderByTitle(selectionScreen.FindChildren("*", owned: false), expectedTitle);

        if (match == null)
        {
            PlayerActionBuffer.LogToDevConsole(
                $"[RunReplays] Replay: card '{expectedTitle}' not found in reward screen.");
            return false;
        }
